...
const Media = (function(){
  ...
  const STORE = 'images';
  let dbPromise = null;
  const cache = new Map();   // id -> dataURL (IDB + data.json publik)
  ...
  async function loadAll(){
    try{
      ...
      return await new Promise((resolve, reject) => {
        ...
        req.onsuccess = () => {
          (req.result || []).forEach(row => cache.set(row.id, row.data));
          resolve(cache);
        };
        req.onerror = () => reject(req.error);
      });
    }catch(err){
      console.warn('Media: gagal membuka IndexedDB, gambar lokal dilewati.', err);
      return cache;
    }
  }
  ...
  async function put(id, dataUrl){
    ...
    try{
      ...
      console.warn('Media: gagal menyimpan gambar.', err);
      return false;
    }
  }
  ...
  async function remove(id){
    ...
    try{
      const store = await tx('readwrite');
      store.delete(id);
    }catch(err){ /* diabaikan — cache sudah bersih */ }
  }
  ...
  function get(id){ return id ? (cache.get(id) || '') : ''; }
  function has(id){ return !!id && cache.has(id); }
  function seed(map){
    if(!map) return;
  ...
  function bytes(){
    let total = 0;
    cache.forEach(value => { total += value.length; });
    return total;
  }

  return { loadAll, put, get, has, remove, clear, seed, all, bytes };
})();
...
function refreshStorageInfo(){
  ...
  el.textContent = 'teks ' + fmt(textBytes) + ' · gambar ' + fmt(imgBytes);
}
